...
cv.error.10 <- cv.glm(df_train,glm.fit,K = 10)$delta[1]
...
# Número de puntos de anclaje (knots) para smooth.spline
knots <- 4:13
...
for (j in 1:length(knots)) {
  ...
  for (i in 1:num_folds) {
    ...
    spline_fit<-smooth.spline(train_data$horsepower,train_data$mpg,nknots=knots[j])
  ...
  ECMCV[j]<- mean(cv.error.10)

    # Imprimir información
  print(paste0("k =", knots[j], ", ECM CV =", ECMCV[j]))
}

# Encuentra el número óptimo de knots con el menor ECM
optimal_knots <- knots[which.min(ECMCV)]

# Imprime los resultados
cat("El número óptimo de knots es:", optimal_knots, "\n")
cat("El ECM mínimo es:", min(ECMCV), "\n")
